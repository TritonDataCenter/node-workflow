...
Workflow.prototype.runTask = function(task, cb) {
  ...
  // Before we set the timeout, when present, set the listener
  self.on('tasktimeout', function(err) {
    onRetryError(err);
  });

  //self.on('tasktimeout', onRetryError);
