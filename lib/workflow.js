...
var Workflow = module.exports = function(job) {
  ...
  if (!this.chain_results) {
    this.chain_results = [];
  }

  if (this.onerror && !this.onerror_results) {
    this.onerror_results = [];
  }
  ...
  this.on('end', function(err, callback) {
    var self = this;
    self.job.chain_results = self.chain_results;
    if (self.onerror_results) {
      self.job.onerror_results = self.onerror_results;
    }
