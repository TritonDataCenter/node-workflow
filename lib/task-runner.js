...
var vm = require('vm');
var createDTrace = require('./index').CreateDTrace;
...
var WorkflowTaskRunner = module.exports = function (opts) {
    ...
    function formatResults(msg) {
        ...
        msg.task_name = name;
        dtp.fire('wf-cp-task-done', function tastkProbeDone() {
            var ret = [
                msg.task_name,
                msg.result,
    ...
    function onRetryError(err, cb) {
        stack.methods.push('onRetryError');
            ...
            if (fallback) {
                ...
                if (timeout) {
                    ...
                    taskFallbackTimeoutId = setTimeout(function () {
                        stack.methods.push('taskFallbackTimeoutId');
                ...
                return fallback(err, job, function (error, result) {
                    ...
                    stack.methods.push('task.fallback callback');
                    // DTrace!
                    dtp.fire('wf-cp-task-done', function taskProbeDone() {
                        var ret = [
                            name,
                            body.toString(),
                            error.toString(),
    ...
    function retryTask(cb) {
        ...
        stack.retries = retries += 1;
        stack.methods.push('retryTask');
        ...
        if (timeout) {
            ...
            taskTimeoutId = setTimeout(function () {
                stack.methods.push('taskTimeoutId');
                retryTimedOut = true;
                process.on('uncaughtException', timeoutListener);
                throw new TimeoutError('task timeout error');
            }, timeout);
        ...
        return body(job, function (err, res) {
            ...
            stack.methods.push('task.body callback');
    ...
    function runTask(callback) {
        ...
        dtp.addProbe('wf-cp-task-start',
                        'char *',
                        'char *',
                        'int');
        // Task name, result, error,
        // end time (time we fire done probe):
        dtp.addProbe('wf-cp-task-done',
                            'char *',
                            'char *',
                            'char *',
                            'int');
        ...
        stack.methods.push('runTask');

        // Go for it!
        dtp.fire('wf-cp-task-start', function taskProbeStart() {
            var ret = [
                name,
                body.toString(),
                new Date().getTime()
