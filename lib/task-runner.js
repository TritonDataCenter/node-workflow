...
var WorkflowTaskRunner = module.exports = function(opts) {
  ...
  // Does the job have a timeout?, how many time remains available to run
  // the whole job?
  this.job_timeout = null;
  if (opts.job.timeout) {
    this.job_timeout = ((opts.job.elapsed) ?
      (opts.job.timeout - opts.job.elapsed) :
      opts.job.timeout) * 1000;
  }
...
WorkflowTaskRunner.prototype.runTask = function(callback) {
  var self = this,
      cb = function(message) {
        ...
        if (self.jobTimeoutId) {
          clearTimeout(self.jobTimeoutId);
          self.jobTimeoutId = null;
        }
  ...
  if (self.job_timeout) {
    self.jobTimeoutId = setTimeout(function() {
      self.stack.methods.push('jobTimeoutId');
      // Exceeded the job's timeout, have to stop running task
      clearTimeout(self.jobTimeoutId);
      self.jobTimeoutId = null;
      // May want to ignore tasks results once we timed out the whole workflow
      self.jobTimedOut = true;
      return callback(self.formatResults({
...
WorkflowTaskRunner.prototype.onRetryError = function(err, cb) {
  ...
  if (self.retries < self.retry) {
    ...
    if (self.fallback) {
      ...
      self.fallback(err, self.job, function(error, result) {
        if (self.taskFallbackTimeoutId) {
          process.removeListener('uncaughtException', self.timeoutListener);
          clearTimeout(self.taskFallbackTimeoutId);
          self.taskFallbackTimeoutId = null;
