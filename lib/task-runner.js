...
var WorkflowTaskRunner = module.exports = function (opts) {
                ...
                if (k === 'modules') {
                    ...
                    if (opts.task.modules) {
                        ...
                        Object.keys(opts.task.modules).forEach(
                            ...
                            if (opts.task.modules[mod] === 'restify') {
                                // Unconscionable hack, swap in better clients
                                // so that we have backward compatible support
                                // in workflows.
                                //
                                // Ewww.
                                [
                                    ...
                                    'JsonClient',
                                    'StringClient'
                                ].forEach(function _hackRestifyClient(func) {
                                    global[mod][func] = restifyClients[func];
                                });
                            }
    ...
    // If we have an opts.span, replace body with one that's got tracing context
    if (opts.span) {
        origBody = body;
        body = function _tracedBody() {
            var self = this;
            var origArgs = arguments;
            var cls = tritonTracer.cls();

            return cls.runAndReturn(function _runTracedBody() {
                cls.set(tritonTracer.consts.CLS_SPAN_KEY, opts.span);
                return origBody.apply(self, origArgs);
            });
        };
    }
            ...
            if (fallback) {
                ...
                try {
                    return fallback(err, job, function (error, result) {
                        clearTaskTimeoutId(taskFallbackTimeoutId);
                        // If even the error handler returns an error, we have
                        // to bubble it up:
                        if (error) {
                            return cb(formatResults({
                                error: error
                        ...
                        // If the 'fallback' handler fixed the error, let's
                        // return success despite of body failure:
                        return cb(formatResults({
    ...
    function retryTask(cb) {
        ...
        return body(job, function (err, res) {
            ...
            if (err) {
                // A task can re-queue a job:
                if (err === 'queue') {
                    return cb(formatResults({
                        result: (res) ? res : 'OK',
                        error: 'queue'
                    }));
                // Or retry the whole job:
                } else if (err === 'retry') {
                    return cb(formatResults({
                        result: (res) ? res : 'OK',
                        error: 'retry'
                    }));
                // Or tell the job to hold on until something external happens:
                } else if (err === 'wait') {
                    return cb(formatResults({
                        result: (res) ? res : 'OK',
                        error: 'wait'
    ...
    //     of 'run', 'error', 'queue', 'retry'. (In the future we may also
    //     implement 'pause' to let the runner set a timeout to continue
    //     execution).
    function runTask(callback) {
        ...
        try {
            ...
            clearTaskTimeoutId(taskTimeoutId);
            // On this case, we will make an exception and return the stack,
            // b/c this means the task is raising an uncaught exception:
    ...
    function markDone(execution) {
        if (job) {
            job.timeToDie = true;
        }
    }

    taskRunner.markDone = markDone;
