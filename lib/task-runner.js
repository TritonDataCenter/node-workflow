...
var bunyan = require('bunyan'),
    util = require('util'),
    vm = require('vm'),
    createDTrace = require('./index').CreateDTrace;
...
var WorkflowTaskRunner = module.exports = function (opts) {
    ...
    var use_eval = (opts.use_eval) ? opts.use_eval : false;
console.log(use_eval);
    var sandbox = {
    ...
    this.canceled = false;
    // DTrace Provider
    this.dtp = createDTrace('job-task');
};
...
WorkflowTaskRunner.prototype.runTask = function (callback) {
    ...
    self.dtp.addProbe('wf-cp-task-start',
                        'char *',
                        'char *',
                        'int');
    // Task name, result, error,
    // end time (time we fire done probe):
    self.dtp.addProbe('wf-cp-task-done',
                        'char *',
                        'char *',
                        'char *',
                        'int');
    ...
    // Go for it!
    self.dtp.fire('wf-cp-task-start', function taskProbeStart() {
        var ret = [
            self.name,
            self.body.toString(),
            new Date().getTime()
...
WorkflowTaskRunner.prototype.formatResults = function (msg) {
    ...
    // GH-82: If we have an error instance, try to provide useful information,
    // like restify.Error does:
    if (msg.error && typeof (msg.error) !== 'string' && !msg.error.statusCode) {
    ...
    msg.job = {};
    for (var p in self.job) {
        if (p == 'log') {
            continue;
        }
        msg.job[p] = self.job[p];
    }
    ...
    self.dtp.fire('wf-cp-task-done', function tastkProbeDone() {
        var ret = [
            msg.task_name,
            msg.result,
