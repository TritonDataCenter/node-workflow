...
var API = module.exports = function (opts) {
    ...
    // Setup tracing for all handled routes
    ...
    function postJob(req, res, next) {
        ...
        // Add tracing info to job so it can be picked up by runner and so that
        // it's available for debugging.
        if (req.tritonTraceSpan) {
            job.tritonTraceSpanCtx = req.tritonTraceSpan.context();
        }
