...
// 11-03-2015 15:31 - 739/775
// 12-03-2015 14:32 - 689/721
// 12-03-2015 16:03 - 680/717 - finishJob: delete locks
// 12-03-2015 16:03 - 694/721 - validateJobTarget: remove checks without locks
// 12-03-2015 16:03 - 703/722 - queueJob: not given job have to be checked if running
// 13-03-2015 09:44 - 704/722 - corrected typo of error message
// 13-03-2015 11:39 - 708/722 - fixed getJobs finding (params are in a sub-object)
// 13-03-2015 11:39 - 709/722 - return value of removing now counts items
// 16-03-2015 09:30 - 712/722 - removed getRunners error
// 16-03-2015 09:30 - 771/779 - checking for waiting & running jobs was incorrect (should check on db-obj)
// 16-03-2015 09:30 - 784/791 - saving assigned/extended objects back to database so no values getting lost
...
var baseBackend = {
    ...
    validateJobTarget: function (pJob, pCallback) {
        ...
        _self.find(_self.TYPES.JOB, null, function (pError, pJobs) {
            if (pError || !pJobs)
                return pCallback(null);
            //console.error("validateJobTarget() found:%j", pJobs.length);
            if (_.some(pJobs, function (job) {
                    //console.error("validateJobTarget() check:%j;%j==%j", job, job.locks, pJob.target);
                ...
                //console.error("validateJobTarget() locked:%j", pJob);
            ...
            //console.error("validateJobTarget() sameTargets:%j", sameTargets);
    ...
    updateJob: function (pJob, meta, pCallback) {
        ...
        var _self = this;
        //console.error('updateJob(): uuid=%j', pJob.uuid);
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf('Job with uuid \'%s\' does not exist', pJob.uuid)));
            var job = pJobs[0];
            _.assign(job, pJob);
            _self.save(_self.TYPES.JOB, job, pCallback);
        });
        ...
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', pJob.uuid)));
            var job = pJobs[0];
            //console.error('pauseJob(): execution=%j', pJob.execution);
            if (job.execution !== _self.EXECUTION.RUNNING)
                return pCallback(new e.BackendPreconditionFailedError(
    ...
    addInfo: function (uuid, info, meta, pCallback) {
        ...
        var _self = this;
        //console.error('addInfo(): uuid=%j info=%j', uuid, info);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(
    ...
    getInfo: function (uuid, meta, pCallback) {
        ...
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            ...
            //console.error('getInfo(): uuid=%j info=%j', uuid, job.info);
            return pCallback(null, job.info);
        });
    }
};
