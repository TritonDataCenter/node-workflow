...
var baseBackend = {
    ...
    createWorkflow: function (pWorkflow, pMeta, pCallback) {
        ...
        this.find(this.TYPES.WORKFLOW, {'name': pWorkflow.name}, function (pError, pWorkflows) {
            ...
            _self.save(_self.TYPES.WORKFLOW, clone(pWorkflow), function (pError) {
                return pCallback(pError, pWorkflow);
            });
        });
    },
    // usage: API & Factory
    ...
    getWorkflow: function (uuid, meta, pCallback) {
        ...
        this.find(this.TYPES.WORKFLOW, {'uuid': uuid}, function (pError, pWorkflows) {
            ...
            return pCallback(null, clone(pWorkflows[0]));
        });
    },
    // usage: API
            ...
            _self.find(_self.TYPES.WORKFLOW, {'name': pWorkflow.name}, function (pError, pWorkflows) {
                if (pWorkflows.length > 0)
                    for (var i = 0; i < pWorkflows.length; i++) {
                        if (pWorkflows[i].uuid !== workflow.uuid)
                            return pCallback(new e.BackendInvalidArgumentError(
                                'Workflow.name must be unique. A workflow with name "' +
                                workflow.name + '" already exists'));
                    }
                _self.save(_self.TYPES.WORKFLOW, clone(workflow), function (pError) {
                    return pCallback(pError, workflow);
                });
            });
        });
    },
    ...
    createJob: function (job, meta, pCallback) {
        ...
        this.save(this.TYPES.JOB, clone(job), function (pError) {
            return pCallback(pError, job);
        });
    },
    ...
    getJobProperty: function (uuid, prop, pCallback) {
        console.error('getJobProperty(): uuid=%j', uuid);
    ...
    validateJobTarget: function (pJob, pCallback) {
        ...
        var _self = this;
        _self.find(_self.TYPES.JOB, {'locks': pJob.target}, function (pError, pJobs) {
            if (pJobs.length > 0)
                return pCallback(new e.BackendInvalidArgumentError(
                    'Job target is currently locked by another job'));
            _self.find(_self.TYPES.JOB, {'target': pJob.target}, function (pError, pJobs) {
                if (pJobs.length === 0) // needed? own job will be found
                    return pCallback(null);
                for (var i = 0; i < pJobs.length; i++) {
                    var job = pJobs[i];
                    if (job.uuid !== pJob.uuid && job.target === pJob.target &&
                        Object.keys(pJob.params).every(function (p) {
                            return (job.params[p] && job.params[p] === pJob.params[p]);
                        }) && (job.execution === _self.EXECUTION.QUEUED ||
                        job.execution === _self.EXECUTION.RUNNING))
                        return pCallback(new e.BackendInvalidArgumentError('Another job (' + job.uuid + ') with the same target and params is already queued'));
                }
                return pCallback(null);
            });
        });
    ...
    nextJob: function (index, pCallback) {
        ...
        this.find(this.TYPES.JOB, {'execution': this.EXECUTION.QUEUED}, function (pError, pJobs) {
            return pCallback(null, (pJobs.length === 0 || index >= pJobs.length) ? null :
                clone(pJobs.slice(index, index + 1)[0]));
        });
    },
    ...
    runJob: function (uuid, runner_id, pCallback) {
        var _self = this;
        console.error('runJob(): uuid=%j', uuid);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            ...
            job.execution = _self.EXECUTION.RUNNING;
            return _self.save(_self.TYPES.JOB, clone(job), function (pError) {
                return pCallback(pError, job);
            });
        });
    },
    ...
    finishJob: function (pJob, pCallback) {
        var _self = this;
        console.error('finishJob(): uuid=%j', pJob.uuid);
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', pJob.uuid)));
            if (pJobs[0].execution !== _self.EXECUTION.RUNNING &&
                pJobs[0].execution !== _self.EXECUTION.CANCELED)
                return pCallback(new e.BackendPreconditionFailedError(
                    'Only running jobs can be finished'));
            if (pJob.execution === _self.EXECUTION.RUNNING)
                pJob.execution = _self.EXECUTION.SUCCEEDED;
            delete pJob.runner_id;
            _self.save(_self.TYPES.JOB, clone(pJob), function (pError) {
                return pCallback(pError, pJob);
            });
        });
    },
    ...
    //            something fails, otherwise it'll return the updated job
    //            using getJob.
    updateJob: function (job, meta, pCallback) {
        if (typeof (meta) === 'function') {
            pCallback = meta;
            meta = {};
        }
        var _self = this;
        console.error('updateJob(): uuid=%j', job.uuid);
        this.find(this.TYPES.JOB, {'uuid': job.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf('Job with uuid \'%s\' does not exist', job.uuid)));
            _self.save(_self.TYPES.JOB, clone(job), function (pError) {
                return pCallback(pError, job);
            });
        });
    },
    ...
    cancelJob: function (uuid, pCallback) {
        ...
        var _self = this;
        console.error('cancelJob(): uuid=%j', uuid);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf('Job with uuid \'%s\' does not exist', job.uuid)));
            var job = pJobs[0];
    ...
    updateJobProperty: function (uuid, prop, val, meta, pCallback) {
        ...
        var _self = this;
        console.error('updateJobProperty(): uuid=%j', uuid);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', uuid)));
    ...
    queueJob: function (pJob, pCallback) {
        var _self = this;
        console.error('queueJob(): uuid=%j', pJob.uuid);
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', pJob.uuid)));
            ...
            delete pJob.runner_id;
            pJob.execution = _self.EXECUTION.QUEUED;
            _self.save(_self.TYPES.JOB, clone(pJob), function (pError) {
                return pCallback(pError, pJob);
            });
        });
    ...
    pauseJob: function (pJob, pCallback) {
        var _self = this;
        console.error('pauseJob(): uuid=%j', pJob.uuid);
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', pJob.uuid)));
            ...
            delete pJob.runner_id;
            pJob.execution = _self.EXECUTION.WAITING;
            _self.save(_self.TYPES.JOB, clone(pJob), function (pError) {
                return pCallback(pError, pJob);
            });
        });
    ...
    resumeJob: function (pJob, pCallback) {
        var _self = this;
        console.error('resumeJob(): uuid=%j', pJob.uuid);
        this.find(this.TYPES.JOB, {'uuid': pJob.uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(sprintf(
                    'Job with uuid \'%s\' does not exist', pJob.uuid)));
            //var job = pJobs[0];
            if (pJob.execution !== _self.EXECUTION.WAITING)
                return pCallback(new e.BackendPreconditionFailedError(
    ...
    getRunners: function (pCallback) {
        this.find(this.TYPES.RUNNER, null, function (pError, pRunners) {
            ...
            for (var i = 0; i < pRunners.length; i++) {
                theRunners[pRunners.uuid] = pRunners.active_at;
            }
    ...
    getRunnerJobs: function (runner_id, pCallback) {
        this.find(this.TYPES.JOB, {'runner_id': runner_id}, function (pError, pJobs) {
            console.error("getRunnerJobs(): %j", _.map(pJobs, 'uuid'));
    ...
    getWorkflows: function (params, pCallback) {
        ...
        this.find(this.TYPES.WORKFLOW, params, function (pError, pWorkflows) {
            return pCallback(pError, clone(pWorkflows));
        });
    },
    ...
    getJobs: function (params, pCallback) {
        ...
        console.error("getJobs(): params=%j", params);
        this.find(this.TYPES.JOB, params, function (pError, pJobs) {
            if (pError)
                return pCallback(pError, clone(pJobs));
            if (typeof (offset) !== 'undefined' &&
                typeof (limit) !== 'undefined') {
                return pCallback(pError, clone(pJobs.slice(offset, limit)));
            } else {
                return pCallback(pError, clone(pJobs));
            }
        });
    ...
    addInfo: function (uuid, info, meta, pCallback) {
        ...
        var _self = this;
        console.error('addInfo(): uuid=%j', uuid);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(
            ...
            job.info.push(info);
            _self.save(_self.TYPES.JOB, job, function (pError, pJob) {
                return pCallback(pError, clone(pJob));
            });
        });
    },
    ...
    getInfo: function (uuid, meta, pCallback) {
        ...
        console.error('getInfo(): uuid=%j', uuid);
        this.find(this.TYPES.JOB, {'uuid': uuid}, function (pError, pJobs) {
            if (pJobs.length === 0)
                return pCallback(new e.BackendResourceNotFoundError(
            ...
            return pCallback(null, clone(job.info));
        });
    }
};
