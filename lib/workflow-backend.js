...
//       properties.
// runner_id - the identifier of the runner which is telling us this job is
//             done (to keep track of it + make unlock easier).
// callback - f(err) called with error if something fails, otherwise with null.
WorkflowBackend.prototype.finishJob = function(job, runner_id, callback) {
...
// runner_id - the identifier of the runner which is telling us this job needs
//             to be re-queued.
// callback - f(err) same approach, if something fails called with error.
WorkflowBackend.prototype.queueJob = function(job, runner_id, callback) {
