...
WorkflowJobRunner.prototype.run = function (callback) {
  var self = this;
  self.runner.getSlot();
  // Check for job status. If it's canceled, send child process a message
    ...
    self.backend.getJobProperty(
      ...
      function (err, val) {
        if (err) {
          ...
          // We've re-queued the job from runner due to process trying to exit,
          // re-queue for next boot.
          self.canceled = true;
...
WorkflowJobRunner.prototype.runTask = function (task, chain, cb) {
  ...
  if (self.canceled && self.job.execution === 'queued') {
    return cb('queue');
  }
