...
var WorkflowJobRunner = module.exports = function (opts) {
    ...
    function _updateJobProperty(uuid, prop, val, meta, callback) {
        ...
        attempt.on('ready', function (number, delay) {
            console.log('ready arguments ' + util.inspect(arguments));
    ...
    function saveJob(callback) {
        job.elapsed = (new Date().getTime() - started) / 1000;
    ...
    function runTask(task, chain, cb) {
        ...
        function runTaskCallback(msg) {
            ...
            if (!util.isArray(job[chain])) {
                ...
                job[chain].push(res);
                return _updateJobProperty(job.uuid, chain, job[chain],
                  function (err) {
        ...
        taskRunner = new WorkflowTaskRunner({
            task: task,
            job: job,
            sandbox: sandbox,
            trace: log.trace()
        ...
        return taskRunner.runTask(runTaskCallback);
    }
    ...
    function runChain(chain, chain_results, callback) {
        ...
        async.forEachSeries(chain_to_run, function (task, async_cb) {
            ...
            runTask(task, chain_results, async_cb);
            taskRunner = null;
        }, function (err) {
