...
var WorkflowJobRunner = module.exports = function (opts) {
    ...
    function runTask(task, chain, cb) {
        ...
        function runTaskCb(msg) {
            ...
            if (msg.job) {
                job.params = msg.job;
                // Backwards compatibility with versions passing the whole job:
                Object.keys(job.params.params).forEach(function (p) {
                    if (!job.params[p]) {
                        job.params[p] = job.params.params[p];
                    }
                });
                delete job.params.params;
            }
        ...
        var payload = job.params;
        // For backwards compatibility, given previous versions were
        // expecting params to be here:
        payload.params = clone(job.params);
        if (do_fork) {
            try {
