...
var WorkflowJobRunner = module.exports = function (opts) {
    ...
    function runTask(task, chain, cb) {
        ...
        if (job.tritonTraceSpanCtx) {
            try {
                fields.childOf = JSON.parse(job.tritonTraceSpanCtx);
            } catch (e) {
                log.error({
                    err: e,
                    job_uuid: job.uuid
                }, 'failed to parse job trace ctx');
            }
        }
        span = tritonTracer.tracer().startSpan(task.name, fields);
        ...
        function runTaskCb(msg) {
            ...
            span.addTags({
                error: msg.error ? true : undefined,
                errorMsg: msg.error ? msg.error.message : undefined,
                errorName: msg.error ? msg.error.name : undefined,
                errorStatusCode: msg.error ? msg.error.statusCode : undefined,
                job_uuid: job.uuid,
                result: msg.result
            });
            span.finish();
        ...
        if (do_fork) {
            if (span) {
                throw new Error('tracing with forked runners not supported');
            }
