...
WorkflowJobRunner.prototype.runTask = function (task, chain, cb) {
    ...
    self.dtrace.fire('wf-task-start', function taskProbeStart() {
        var ret = [
            task.name,
            task.body,
            new Date().getTime()
    ...
    try {
        ...
        self.child.on('message', function (msg) {
            self.log.debug({message: msg}, 'child process message');
            ...
            if (msg.info) {
                ...
                return self.backend.addInfo(self.job.uuid, info,
                    function (err) {
                    if (err) {
                        self.log.error({err: err}, 'Error adding info');
                    }
                });
            }
            ...
            self.dtrace.fire('wf-task-done', function tastkProbeDone() {
                var ret = [
                    res.name,
                    res.result,
                    res.error,
