...
// Get all the tasks:
// - callback - f(err, tasks)
WorkflowRedisBackend.prototype.getTasks = function(callback) {
  var self = this,
      multi = self.client.multi();

  self.client.smembers('wf_tasks', function(err, res) {
    if (err) {
      return callback(err);
    }
    res.forEach(function(uuid) {
      multi.hgetall('task:' + uuid);
    });
    multi.exec(function(err, replies) {
      if (err) {
        return callback(err);
