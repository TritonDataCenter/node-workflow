...
//               sandbox.
var WorkflowRunner = module.exports = function (opts, backend) {
  ...
  this.run_interval = (opts.runner.run_interval || 2) * 60 * 1000;
  this.interval = null;
          ...
          async.parallel({
            ...
            stale_jobs: function (cb) {
              self.staleJobs(function (err, jobs) {
                if (err) {
                  self.log.error('Error fetching stale jobs: ' + err);
