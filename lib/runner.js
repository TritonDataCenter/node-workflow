...
var WorkflowRunner = module.exports = function (opts) {
    ...
    // XXX perhaps artedi collector should be passed in? or the labels for it?
    var metricsCollector = artedi.createCollector({
        labels: {
            // XXX hacked in my values
            datacenter: 'coal',
            server: '564d8f85-5be0-dbc8-2572-ac66302f0d0a',
            zonename: '6eca6690-b18f-4513-85a9-0b3d114cec12',
            pid: process.pid
        }
    });
    var monitorServer = restify.createServer({ serverName: 'Monitor' });
    monitorServer.get('/metrics', function getMetrics(req, res, next) {
        // XXX why this on `req.on('end', ...)`?
        req.on('end', function () {
            metricsCollector.collect(artedi.FMT_PROM, function (err, metrics) {
                if (err) {
                    next(new VError(err, 'error retrieving metrics'));
                    return;
                }
                // XXX why the version=0.0.4 for content negotiation
    ...
    // XXX why 0.0.0.0?! I'd limit to configured ADMIN_IP or separate config var.
    var port = 8881; // or parameterized
    monitorServer.listen(port, '0.0.0.0', function () {
        // XXX use addr arg to show where listening
        log.info('monitoring server started on port %d', port);
    });
    opts.backend.opts.metricsCollector = metricsCollector;
