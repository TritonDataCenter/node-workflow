...
WorkflowRunner.prototype.init = function(callback) {
  ...
  // 3) Then the runner will tell the backend to retrieve the next queued
  //    job. If the job 'exec_after' attribute is smaller than the current
  //    time, it'll execute the job (set the job as running, lock it as
  //    running by the runner). If the 'exec_after' attribute is greater than
  //    the current time, it will tell the backend to retrieve the next
  //    queued job.
  // 4) Runners can spawn processes. The number of spawned processes should
  //    be configurable.
  // 5) Once a runner gets a job to run, and right after lock it, it'll pass
  //    such job to the spawned child process, and wait for a child message.
  //    The message may be one of: error, success or re-queue the job. The
  //    job will be always included on the message, to make its status be
  //    persisted into the backend. In theory, the child process should know
  //    nothing at all about the backend, just run the job tasks, given all
  //    the required information should be contained at the job.
...
WorkflowRunner.prototype.runJob = function(job, callback) {
  ...
  child.on('message', function(msg) {
    ...
    // Decide what to do with the Job depending on its status:
    if (msg.job.status === 'finished') {
