    ...
    self.backend.getRunnerJobs(self.identifier, function (err, jobs) {
      if (err) {
        ...
        return async.forEach(jobs, function (uuid, next_cb) {
          self.backend.updateJobProperty(uuid, 'execution', 'canceled',
            function (err) {
              if (err) {
                return next_cb(err);
              }
...
WorkflowRunner.prototype.quit = function (callback) {
  ...
  if (self.running_jobs.length > 0) {
    self.running_jobs.forEach(function (j) {
      self.log.trace('Job with UUID ' + j + ' queued for cancelation');
      serie.push(function (next) {
        self.backend.updateJobProperty(
          j, 'execution', 'queued', function (err) {
            if (err) {
              return next(err);
            }
  ...
  async.series(serie, function (err, res) {
    if (err) {
      self.log.error({error: err}, 'Error trying to cancel job');
    }
...
WorkflowRunner.prototype.run = function () {
  ...
  worker = function (uuid, callback) {
    return self.backend.getJob(uuid, function (err, job) {
      ...
      if (self.runNow(job)) {
        return self.backend.runJob(uuid, self.identifier, function (err, job) {
          if (err) {
            return callback(err);
          }
          ...
          async.parallel({
            ...
            stale_jobs: function (cb) {
              self.staleJobs(function (err, jobs) {
                if (err) {
                  self.log.error({err: err}, 'Error fetching stale jobs');
            ...
            fetch_jobs: function (cb) {
              ...
              if (isNaN(fetch) || fetch <= 0) {
                self.log.info('No available slots. Waiting next iteration');
                return cb(null, null);
              }
              return self.backend.nextJobs(0, fetch, function (err, jobs) {
                // Error fetching jobs
                if (err) {
                  self.log.error({err: err}, 'Error fetching jobs');
                  // We will not stop even on error:
                  return cb(null, null);
                }
                // No queued jobs
                ...
                jobs.forEach(function (job) {
                  if (self.running_jobs.indexOf(job) === -1) {
                    self.running_jobs.push(job);
                    queue.push(job, function (err) {
                      // Called once queue worker finished processing the job
                      if (err) {
                        self.log.error({err: err}, 'Error running job');
                      }
                      self.log.info('Job with uuid ' + job +
                        ' ran successfully');
                      if (self.running_jobs.indexOf(job) !== -1) {
                        self.running_jobs.splice(
                          self.running_jobs.indexOf(job), 1);
                      }
                    });
                  }
                });
