    ...
    function run() {
        ...
        function retryJob(oldJob, callback) {
            if (oldJob.num_attempts + 1 >= oldJob.max_attempts) {
                return callback(
                  'Max attempts reached. Last attempt: ' + oldJob.uuid, oldJob);
            }
            ...
            backend.isRunnerIdle(identifier, function (idle) {
                ...
                vasync.parallel({
                    ...
                    funcs: [function cancelStaleJobs(cb) {
                        staleJobs(function (err, jobs) {
                            ...
                            function cancelJobs(uuid, fe_cb) {
                                backend.updateJobProperty(uuid, 'execution',
                                    ...
                                    return backend.getJob(uuid,
                                        ...
                                        return backend.finishJob(job,
                                            function (err, job) {
                                            if (err) {
                                                return fe_cb(err);
                                            }
                                            log.info('Stale Job %s concelled',
                                                job.uuid);
                                            return fe_cb(null);
                                        });
                                    });
                    ...
                    function fetchJobsToProcess(cb) {
                        ...
                        return backend.nextJobs(0, fetch, function (err, jobs) {
                            // Error fetching jobs
                            if (err) {
                                ...
                                if (rn_jobs.indexOf(job) === -1) {
                                    ...
                                    queue.push(job, function (err, j) {
                                        ...
                                        if (err) {
                                            ...
                                            log.info('Job with uuid %s ' +
                                                'ran successfully', job);
                                        }
                                        if (rn_jobs.indexOf(job) !== -1) {
                                            rn_jobs.splice(rn_jobs.indexOf(job),
                                                1);
                                        }

                                        // fire callback urls
                                        if (j && j.callback_urls) {
                                            j.callback_urls.forEach(
                                                function (uri) {
                                                var u = url.parse(uri);
                                                u.method = 'POST';
                                                u.headers = {
                                                ...
                                                var req = http.request(u, function(res) {
                                                    var logLevel = (res.statusCode >= 200 && res.statusCode < 300) ? 'info' : 'warn';
                                                    log[logLevel]({job: job, uri: uri, code: res.statusCode}, 'POST to callback URL %s', uri);
                                                });
                                                req.on('error', function(err) {
                                                    log.warn({uri: uri, job: job, err: err}, 'failed to POST to callback URL %s', uri);
                                                });
